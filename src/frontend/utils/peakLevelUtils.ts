/**
 * Peak level ordering: module > E2 > base.
 * Used to filter to one entry per operator (peak only) when level badges are hidden.
 */

const RATING_ORDER = ['SS', 'S', 'A', 'B', 'C', 'D', 'F'] as const;
...
export function keepPeakInstanceOnly<T extends InstanceLike>(instances: T[]): T[] {
  ...
  for (let i = 1; i < instances.length; i++) {
    ...
    if (curLevel > bestLevel || (curLevel === bestLevel && curTier < bestTier)) {
      best = cur;
    }
  }
  return [best];
