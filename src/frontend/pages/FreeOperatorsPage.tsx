...
const FreeOperatorsPage: React.FC = () => {
  const { language } = useLanguage();
  const { user } = useAuth();
  const { t, getNicheName, getNicheDescription } = useTranslation();
  const [data, setData] = useState<FreeOperatorsData | null>(null);
  ...
  const loadFreeOperators = async () => {
    try {
      const response = await apiFetch('/api/free-operators');
      if (!response.ok) {
      ...
      setError(err instanceof Error ? err.message : 'Failed to load free operators');
      setLoading(false);
    }
  };

  if (loading) {
    return <div className="loading">Loading free operators...</div>;
  }

  if (error || !data) {
    return <div className="error">{error || 'Free operators data not found'}</div>;
  }
  ...
  return (
    <div className="free-operators-page">
      <div className="free-operators-header">
        <Link to="/tier-lists" className="back-button">
          {t('nicheList.backToTierLists')}
        </Link>
        <h1>{getNicheName('free', data.niche)}</h1>
        <p>{getNicheDescription('free', data.description)}</p>
        <div className="meta">
          {t('common.lastUpdated')}: {data.lastUpdated} • {data.operators.length} {t('common.operators')}
        </div>
      </div>

      <div className="free-operators-container">
...
export default FreeOperatorsPage;
